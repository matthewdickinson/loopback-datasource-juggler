...
DataSource.prototype.defineForeignKey = function defineForeignKey(className, key, foreignClassName, pkName) {
  var addFkToSettings = function(model, key, foreignModel, pkName) {
    if (!model.settings.foreignKeys) model.settings.foreignKeys = {};

    var fkName = 'fk_' + model.name.toLowerCase() + '_' + key;
    model.settings.foreignKeys[fkName] = {
      name: fkName,
      entity: foreignModel,
      entityKey: pkName,
      foreignKey: key,
  ...
  addFkToSettings(model, key, foreignModel, pkName);
